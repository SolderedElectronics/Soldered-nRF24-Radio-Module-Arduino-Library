...
void setup(void)
{
    ...
    // csn is used in SPI transfers. Set to LOW at start and HIGH after transfer. Set to HIGH to reflect no transfer
    // active SPI command are accepted in Power Down state. CE pin represent PRX (LOW) or PTX (HIGH) mode apart from
    // register settings. Start in PRX mode.
    ce(LOW);
    csn(HIGH);

    // nRF24L01 goes from to Power Down state 100ms after Power on Reset ( Vdd > 1.9V) or when PWR_UP is 0 in config
    // register Goto Power Down state (Powerup or force) and set in transmit mode
...
void loop(void)
{
} // this program runs only once, thus it resides in setup()
